...
float3 SwapchainPass(float3 color) {
  ...
  color = LinearizeAndClampMaxChannel(color);
...
float3 ApplyDisplayMapAndScale(float3 color_gamma, float2 grain_uv) {
  float3 color;
  if (RENODX_GAMMA_CORRECTION == 2.f) {
    color = renodx::color::srgb::DecodeSafe(color_gamma);
    color = ApplyGammaCorrectionByLuminosity(color);
  } else if (RENODX_GAMMA_CORRECTION == 1.f) {
    color = renodx::color::gamma::DecodeSafe(color_gamma);
  } else {
    color = renodx::color::srgb::DecodeSafe(color_gamma);
  }
  ...
  if (RENODX_TONE_MAP_TYPE != 0.f) {
    ...
    if (RENODX_TONE_MAP_TYPE == 2.f) {
      color = renodx::tonemap::neutwo::MaxChannel(color, RENODX_PEAK_WHITE_NITS / RENODX_DIFFUSE_WHITE_NITS);
    }
  ...
  if (CUSTOM_GRAIN_TYPE != 0.f) {
    color = renodx::effects::ApplyFilmGrain(color, grain_uv, CUSTOM_RANDOM, CUSTOM_GRAIN_STRENGTH * 0.03f);
  }

  color *= RENODX_DIFFUSE_WHITE_NITS / RENODX_GRAPHICS_WHITE_NITS;

  if (RENODX_GAMMA_CORRECTION == 0.f) {
    color_gamma = renodx::color::srgb::EncodeSafe(color);
  } else {
    color_gamma = renodx::color::gamma::EncodeSafe(color);
  }
  return color_gamma;
}
