...
void main(
  ...
  float3 untonemapped_ap1 = float3(_898, _899, _900);
  ...
  if (RENODX_TONE_MAP_TYPE != 0.f) {
    // tonemapped_ap1 = DeltaToneMap(untonemapped_ap1, tonemapped_ap1, _767, _768, _769);

    float3 sdr_tonemap_ap1 = tonemapped_ap1;
    _898 = 0.18f, _899 = 0.18f, _900 = 0.18f;
    _767 = 0.18f, _768 = 0.18f, _769 = 0.18f;

    _916 = (FilmBlackClip + 1.0f) - FilmToe;
    _918 = FilmWhiteClip + 1.0f;
    _920 = _918 - FilmShoulder;
    if (FilmToe > 0.800000011920929f) {
      _938 = (((0.8199999928474426f - FilmToe) / FilmSlope) + -0.7447274923324585f);
    } else {
      float _929 = (FilmBlackClip + 0.18000000715255737f) / _916;
    ...
    _941 = ((1.0f - FilmToe) / FilmSlope) - _938;
    _943 = (FilmShoulder / FilmSlope) - _941;
    ...
    _953 = FilmSlope * (_947 + _941);
    _954 = FilmSlope * (_948 + _941);
    _955 = FilmSlope * (_949 + _941);
    _956 = _916 * 2.0f;
    _958 = (FilmSlope * -2.0f) / _916;
    _959 = _947 - _938;
    _960 = _948 - _938;
    _961 = _949 - _938;
    _980 = _920 * 2.0f;
    _982 = (FilmSlope * 2.0f) / _920;
    _1007 = select((_947 < _938), ((_956 / (exp2((_959 * 1.4426950216293335f) * _958) + 1.0f)) - FilmBlackClip), _953);
    _1008 = select((_948 < _938), ((_956 / (exp2((_960 * 1.4426950216293335f) * _958) + 1.0f)) - FilmBlackClip), _954);
    _1009 = select((_949 < _938), ((_956 / (exp2((_961 * 1.4426950216293335f) * _958) + 1.0f)) - FilmBlackClip), _955);
    _1016 = _943 - _938;
    ...
    _1071 = (ToneCurveAmount * (max(0.0f, (lerp(_1051, _1048, 0.9300000071525574f))) - _767)) + _767;
    _1072 = (ToneCurveAmount * (max(0.0f, (lerp(_1051, _1049, 0.9300000071525574f))) - _768)) + _768;
    _1073 = (ToneCurveAmount * (max(0.0f, (lerp(_1051, _1050, 0.9300000071525574f))) - _769)) + _769;
    ...
    // blend the SDR tonemap with untonemapped
    float mid_gray = renodx::color::y::from::AP1(float3(_1089, _1090, _1091));
    float3 untonemapped_scaled_ap1 = untonemapped_ap1 * (mid_gray / 0.18f);
    untonemapped_scaled_ap1 = HueCorrectAP1(untonemapped_scaled_ap1, sdr_tonemap_ap1, 0.25f);
    float3 hdr_tonemap_ap1 = renodx::color::ap1::from::BT709(applyDICE(renodx::color::bt709::from::AP1(untonemapped_scaled_ap1), RENODX_PEAK_NITS, RENODX_DIFFUSE_WHITE_NITS));

    float3 blended_tonemap_ap1 = lerp(sdr_tonemap_ap1, hdr_tonemap_ap1, saturate(sdr_tonemap_ap1));

    _1089 = blended_tonemap_ap1.x, _1090 = blended_tonemap_ap1.y, _1091 = blended_tonemap_ap1.z;
  }

  float _1113 = mad((WorkingColorSpace_FromAP1[0].z), _1091, mad((WorkingColorSpace_FromAP1[0].y), _1090, ((WorkingColorSpace_FromAP1[0].x) * _1089)));
  float _1114 = mad((WorkingColorSpace_FromAP1[1].z), _1091, mad((WorkingColorSpace_FromAP1[1].y), _1090, ((WorkingColorSpace_FromAP1[1].x) * _1089)));
  float _1115 = mad((WorkingColorSpace_FromAP1[2].z), _1091, mad((WorkingColorSpace_FromAP1[2].y), _1090, ((WorkingColorSpace_FromAP1[2].x) * _1089)));
  // _1113 = max(0, _1113);
  // _1114 = max(0, _1114);
  // _1115 = max(0, _1115);
  ...
  float _1164 = lerp(_1141, OverlayColor.x, OverlayColor.w);
  float _1165 = lerp(_1142, OverlayColor.y, OverlayColor.w);
  float _1166 = lerp(_1143, OverlayColor.z, OverlayColor.w);

  // _1164 = pow(max(0, _1164), InverseGamma.y);
  // _1165 = pow(max(0, _1165), InverseGamma.y);
  // _1166 = pow(max(0, _1166), InverseGamma.y);
  ...
  // }
  float3 gamma_color = renodx::color::srgb::EncodeSafe(float3(_1190, _1191, _1192));

  gamma_color = GameScale(gamma_color);

  RWOutputTexture[int3((uint)(SV_DispatchThreadID.x), (uint)(SV_DispatchThreadID.y), (uint)(SV_DispatchThreadID.z))] = float4(gamma_color * (1.f / 1.05f), 0.0f);
}
