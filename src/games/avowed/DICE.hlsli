...
float3 applyDICE(float3 untonemapped, float peak_nits, float paper_white) {
  // Declare DICE parameters
  DICESettings config = DefaultDICESettings();
  config.Type = 3u;
  config.ShoulderStart = 0.25f;
  config.DesaturationAmount = 1.f;
  config.DarkeningAmount = 0.f;

  const float dicePaperWhite = paper_white / renodx::color::srgb::REFERENCE_WHITE;
  const float dicePeakWhite = peak_nits / renodx::color::srgb::REFERENCE_WHITE;

  // multiply paper white in for tonemapping and out for output
  return renodx::color::correct::GammaSafe(DICETonemap(renodx::color::correct::GammaSafe(untonemapped) * dicePaperWhite, dicePeakWhite, config) / dicePaperWhite, true);
}
